...
<section class="panel panel-primary">
							<header class="panel-heading">
								...
								<h5>Master Data Set : Master Loading Port</h5>
							</header>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header bg-success">
        ...
        <h5 class="modal-title" id="exampleModalLabel">Create Master Loading Port</h5>
        
      </div>
      <div class="modal-body">

	  <form class="form-horizontal form-bordered form-bordered" action="<?php echo base_url('DataSetLoadport/createnewloadingport');?>" id="inputdatagroup" method="POST">

	  										<div class="form-group">
												<label class="col-md-3 control-label" for="textareaDefault">Loading Port Code Id</label>
												<div class="col-md-8">
													<input class="form-control" name="loadportdescid" data-plugin-maxlength maxlength="50" required autocomplete="off" style="text-transform: uppercase;" oninput="this.value = this.value.replace(/[^a-z]/g, '').replace(/(\..*?)\..*/g, '$1');" />
													<p>
											...
											<div class="form-group">
												<label class="col-md-3 control-label" for="textareaDefault">Loading Port Description</label>
												<div class="col-md-8">
													<input class="form-control" name="loadportdesc" data-plugin-maxlength maxlength="50" required autocomplete="off" style="text-transform: uppercase;" oninput="this.value = this.value.replace(/[^a-z, ]/g, '').replace(/(\..*?)\..*/g, '$1');" />
													<p>
														<code>max-length</code> set to 50 character <code>uppercase</code>.
													</p>
												</div>
...
<!-- End Modal Insert Group -->		
								...
								<table class="table table-bordered table-striped mb-none" id="tables3">
									<thead>
										<tr>
											<th>No</th>
											<th>Loading Port Code Id</th>
											<th>Loading Port Description</th>
											<th>Loading Port Status</th>
...
<?php
if(!empty($lsmasterloadport)){
  // Jika data pada database tidak sama dengan empty (alias ada datanya)
$no_urut=1;
  foreach($lsmasterloadport as $gaa){
    echo "<tr>";
	echo "<th scope='row'>".$no_urut++."</th>";
	echo "<td>".$gaa->LoadPortDescId."</td>";
	echo "<td>".$gaa->LoadPortDesc."</td>";
	// echo "<td>".$gaa->CountryName."</td>";
	echo "<td>";
		if($gaa->LoadPortStats==1){
			echo "Aktif";
		}else{
			echo "Tidak Aktif";
		}
    ...
    echo "<td style='text-align:center; font-size:18px;'><a style='font-size:18px;' class='btn small-btn-ic-info shadow-sm' href='".base_url("DataSetLoadport/Editmasterloadport/".$gaa->LoadPortId)."'><i class='fa fa-pencil-square-o'></i></a></td>";
    echo "</tr>";
  }
