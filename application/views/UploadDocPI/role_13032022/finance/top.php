<form method="POST" action="<?php echo base_url('MenuDocPI/upload03/'.$getrowspi_value->sign_pi_noid); ?>" enctype="multipart/form-data" class="form-pi">
    <div class="form-group">
        ...
        <div class="col-md-2">
            <?php if($getrowspi_value->sign_pi_top=='1'){ ?>
                <input type="date" name="date03" value="<?php echo $getrowspi_value->sign_pi_top_date ?>" class="form-control" id="inputDefault" <?=($groupname->GroupDesc == 'FINANCE'?'':'disabled')?> >
            <?php }else{ ?>
                <input type="date" name="date03" class="form-control" id="inputDefault" <?=($groupname->GroupDesc == 'FINANCE'?'':'disabled')?> >
            <?php } ?>
        </div>
        <div class="col-md-2">
            <?php if($getrowspi_value->sign_pi_top=='1'){ ?>
                <select id="basic" data-plugin-selectTwo class="form-control" name="top" <?=($groupname->GroupDesc == 'FINANCE'?'':'disabled')?> >
