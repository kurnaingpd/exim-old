<form method="POST" action="<?php echo base_url('MenuDocPI/upload06/'.$getrowspi_value->sign_pi_noid); ?>" enctype="multipart/form-data" class="form-pi">
    <div class="form-group">
        ...
        <div class="col-md-2">
            <?php if($getrowspi_value->sign_shipping_term=='1'){ ?>
                <input type="date" class="form-control" name="date06" value="<?php echo $getrowspi_value->sign_shipping_term_date ?>" id="inputDefault" <?=($groupname->GroupDesc == 'SALES'?'':'disabled')?> required>
            <?php }else{ ?>
                <input type="date" class="form-control" name="date06" id="inputDefault" <?=($groupname->GroupDesc == 'SALES'?'':'disabled')?> required>
            <?php } ?>
        </div>
        <div class="col-md-2">
            <?php if($getrowspi_value->sign_shipping_term=='1'){ ?>
                <select id="basic" class="form-control" name="shipterm" <?=($groupname->GroupDesc == 'SALES'?'':'disabled')?> required>
