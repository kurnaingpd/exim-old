<form method="POST" action="<?php echo base_url('MenuDocPI/upload04/'.$getrowspi_value->sign_pi_noid); ?>" enctype="multipart/form-data" class="form-pi">
    <div class="form-group">
        ...
        <div class="col-md-2">
            <?php if($getrowspi_value->sign_dp_receipt=='1'){ ?>
                <input type="date" class="form-control" name="date04" value="<?php echo $getrowspi_value->sign_dp_receipt_date ?>" id="inputDefault" <?=($groupname->GroupDesc == 'FINANCE'?'':'disabled')?> required>
            <?php }else{ ?>
                <input type="date" class="form-control" name="date04" id="inputDefault" <?=($groupname->GroupDesc == 'FINANCE'?'':'disabled')?> required>
            <?php } ?>
        </div>
        <div class="col-md-2">
