<form method="POST" action="<?php echo base_url('MenuDocPI/upload05/'.$getrowspi_value->sign_pi_noid); ?>" enctype="multipart/form-data" class="form-pi">
    <div class="form-group">
        ...
        <div class="col-md-2">
            <?php if($getrowspi_value->sign_dp_receipt_conf=='1'){ ?>
                <input type="date" class="form-control" name="date05" value="<?php echo $getrowspi_value->sign_dp_receipt_conf_date ?>" id="inputDefault" <?=($groupname->GroupDesc == 'FINANCE'?'':'disabled')?> required>
            <?php }else{ ?>
                <input type="date" class="form-control" name="date05" id="inputDefault" <?=($groupname->GroupDesc == 'FINANCE'?'':'disabled')?> required>
            <?php } ?>
        </div>
        <div class="col-md-2">
