<form method="POST" action="<?php echo base_url('MenuDocPI/upload24/'.$getrowspi_value->sign_pi_noid); ?>" enctype="multipart/form-data" class="form-pi">
    <div class="form-group">
        ...
        <div class="col-md-2">
            <input type="date" class="form-control" name="date24" id="inputDefault" value="<?php echo $getrowspi_value->sign_balancepay_date ?>" <?=($groupname->GroupDesc == 'FINANCE'?'':'disabled')?> required>
        </div>
        <div class="col-md-2">
            <?php if($getrowspi_value->sign_balancepay=='1'){ ?>
                <select id="basic" class="form-control" name="balancepay_val" <?=($groupname->GroupDesc == 'FINANCE'?'':'disabled')?> required>
