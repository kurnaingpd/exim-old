<form method="POST" action="<?php echo base_url('MenuDocPI/upload18/'.$getrowspi_value->sign_pi_noid); ?>" enctype="multipart/form-data" class="form-pi">
    <div class="form-group">
        ...
        <div class="col-md-2">
            <input type="date" class="form-control" name="date18" value="<?php echo $getrowspi_value->sign_expprint_trial_date ?>" id="inputDefault" <?=($groupname->GroupDesc == 'PPIC'?'':'disabled')?> required>
        </div>
        <div class="col-md-2">
            <input type="file" class="form-control" name="berkas" id="inputDefault" <?=($groupname->GroupDesc == 'PPIC'?'':'disabled')?> required>
        </div>
        <div class="col-md-2">
