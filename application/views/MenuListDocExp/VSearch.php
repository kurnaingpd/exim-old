...
<section class="panel panel-primary">
							<header class="panel-heading">
								...
								<h5>Ketentuan Export</h5>
							</header>
							<div class="panel-body">

<div class="container-fluid">

<a class="btn btn-warning" href="<?php echo base_url('MenuListDocExp');?>"><i class="fa fa-arrow-left"></i> Back</a>

</div>

							<hr>

							<form class="form-horizontal form-bordered" method="POST" action="<?php echo base_url('MenuListDocExp/Search');?>">
								<div class="form-group">
									<label class="col-md-1 control-label" for="inputDefault">PI Number</label>
									<div class="col-md-3">
										<input type="text" class="form-control" id="inputDefault" name="pi">
									</div>
								</div>
								...
								<div class="form-group">
									...
									<div class="col-md-2">
										<input type="date" class="form-control" id="inputDefault" name="pi_date_start">
									</div>
									<label class="col-md-1 control-label" for="inputDefault">To</label>
									<div class="col-md-2">
										<input type="date" class="form-control" id="inputDefault" name="pi_date_end">
									</div>
								</div>

								<div class="form-group">
									<label class="col-md-1 control-label" for="inputDefault">Status Upload</label>
									<div class="col-md-3">
								...
								<table class="table table-bordered table-striped mb-none" id="tables3">
									<thead>
										<tr>
											...
											<th class="text-center">PI Numbers</th>
											<th class="text-center">PI Date</th>
											<th class="hidden-phone text-center">Customer</th>
											<th class="hidden-phone text-center">Country</th>
											<th class="hidden-phone text-center">Remark</th>
											<th class="hidden-phone text-center">Status Upload ExpReq</th>
											<th class="hidden-phone text-center">Download Doc 01</th>
											<th class="hidden-phone text-center">Download Doc 02</th>
											<th class="hidden-phone text-center">Actions</th>
											<th class="hidden-phone text-center">Revisi Noted</th>
											<th class="hidden-phone text-center">Revisi Time</th>
										</tr>
									</thead>
									<tbody>
<?php
if(!empty($listexpdataPI)){
  // Jika data pada database tidak sama dengan empty (alias ada datanya)
$no_urut=1;
  foreach($listexpdataPI as $gaa){
    echo "<tr>";
	echo "<th scope='row'>".$no_urut++."</th>";
	echo "<td>".$gaa->gexp_pi_no."</td>";
	echo "<td>".$gaa->gexp_pi_date."</td>";
	echo "<td>".$gaa->CustCompanyName."</td>";
	echo "<td>".$gaa->CountryName."</td>";

	echo "<td>".$gaa->gexp_expreq_remark."</td>";
	
	if($gaa->gexp_expreq_statusDoc==1){
		echo "<td><span class='label label-success'>Approved</span></td>";
	}elseif($gaa->gexp_expreq_statusDoc==2){
		echo "<td><span class='label label-danger'>Void</span></td>";
	}elseif($gaa->gexp_expreq_statusDoc=='3'){
		echo "<td><span class='label label-warning'>Waiting</span></td>";
	}elseif($gaa->gexp_expreq_statusDoc=='4'){
		echo "<td><span class='label label-warning'>Revised</span></td>";
	}else{
		echo "<td><span class='label label-default'>Draft</span></td>";
		// echo "<td><a style='font-size:18px;' href='".base_url("MenuCheckDocexp/upload_expreq/".$gaa->gexp_pi_id)."' class='btn small-btn-ic-info shadow-sm' id='anim' href='#'><i class='fa fa-upload'></i></a></td>";
	}
		// echo "<td>-</td>";
		// echo "<td>-</td>";
	// echo"</span></td>";
	// echo "<td>".$gaa->gexp_pi_ApprovedAt."</td>";	
    // echo "<td><a style='font-size:18px;' href='".base_url("MenuApprovalPI/PreviewPI/".$gaa->gexp_pi_id)."' class='btn small-btn-ic-info shadow-sm' id='anim' href='#'><i class='fa fa-file'></i></a></td>";

    if($gaa->gexp_expreq_docval=='1'){
        echo "<td><a style='font-size:18px;' class='btn small-btn-ic-info shadow-sm' id='anim' href='".base_url("MenuListDocExp/download_expreq/".$gaa->gexp_expreq_id)."'><i class='fa fa-download'></i></a></td>";
    }else{
        echo "<td>-</td>";
	}

	if($gaa->gexp_expreq_doc02!=''){
		if($gaa->gexp_expreq_docval==1){
			echo "<td><a style='font-size:18px;' href='".base_url("MenuCheckDocexp/download_expreq2/".$gaa->gexp_expreq_id)."' class='btn small-btn-ic-info shadow-sm' id='anim' href='#'><i class='fa fa-download'></i></a></td>";
		}else{
			echo "<td>-</td>";
		}
	}else{
	...
	if($gaa->gexp_expreq_docval=='1' AND $gaa->gexp_expreq_statusDoc=='3'){
        echo "<td><a style='font-size:14px;' class='btn small-btn-ic-info shadow-sm' id='anim' href='".base_url("MenuListDocExp/approved_reqexp/".$gaa->gexp_expreq_id)."'><i class='fa fa-check-square-o'></i> Approved</a> <a href='".base_url("MenuListDocExp/void_reqexp/".$gaa->gexp_expreq_id)."' class='btn small-btn-ic-info shadow-sm'><i class='fa fa-times'></i> Void</a> <a href='".base_url("MenuListDocExp/preview_revised/".$gaa->gexp_expreq_id)."' class='btn small-btn-ic-info shadow-sm'><i class='fa fa-recycle'></i> Revised</a></td>";
    }else{
        echo "<td>-</td>";
	}
	...
	if($gaa->gexp_expreq_isRevisi==1){
		echo "<td>".$gaa->gexp_expreq_ValRevisi."</td>";
	}else{
		echo "<td>-</td>";
	}

	if($gaa->gexp_expreq_isRevisi==1){
		echo "<td>".$gaa->gexp_expreq_DateRevisi."</td>";
	}else{
		echo "<td>-</td>";
	}
...
?>
<!-- 
<?php if(isset($listexpdataPI)) : ?>																
	<?php $no_urut = 1; foreach($listexpdataPI as $gaa) : ?>
	<tr>
		<td class="text-center"><?=$no_urut;?></td>
		<td class="text-center"><?=$gaa->pi_no;?></td>
		<td class="text-center"><?=$gaa->pi_date;?></td>
		<td><?=$gaa->cust_name;?></td>
		<td><?=$gaa->country_name;?></td>
		<td><?=$gaa->exp_remark;?></td>
		<td class="text-center"><?=$gaa->exp_doc_status;?></td>
		<td class="text-center">
			<a style='font-size:18px; <?=$gaa->exp_doc_val_1;?>' class='btn small-btn-ic-info shadow-sm' id='anim' href='<?=base_url("MenuListDocExp/download_expreq/".$gaa->exp_id)?>' ><i class='fa fa-download'></i></a>
		</td>
		<td class="text-center">
			<a style='font-size:18px; <?=$gaa->exp_doc_val_2;?>' class='btn small-btn-ic-info shadow-sm' id='anim' href='<?=base_url("MenuListDocExp/download_expreq/".$gaa->exp_id)?>'>
				<i class='fa fa-download'></i>
			</a>
		</td>
		<td class="text-center">
			<a style='font-size:14px; <?=$gaa->actions;?>' class='btn small-btn-ic-info shadow-sm' id='anim' href='<?=base_url("MenuListDocExp/approved_reqexp/".$gaa->exp_id)?>'>
				<i class='fa fa-check-square-o'></i> Approved
			</a>
			<a style="<?=$gaa->actions;?>" href='<?=base_url("MenuListDocExp/void_reqexp/".$gaa->gexp_expreq_id)?>' class='btn small-btn-ic-info shadow-sm'>
				<i class='fa fa-times'></i> Void
			</a>
			<a style="<?=$gaa->actions;?>" href='<?=base_url("MenuListDocExp/preview_revised/".$gaa->gexp_expreq_id)?>' class='btn small-btn-ic-info shadow-sm'>
				<i class='fa fa-recycle'></i> Revised
			</a>
		</td>
		<td class="text-center"><?=$gaa->rev_notes;?></td>
		<td class="text-center"><?=$gaa->rev_dates;?></td>
	</tr>
