<?php
class MLogin_Import extends CI_Model
{
...
{
    // $this->db->reconnect();
    $qry="SELECT * FROM gimp_user WHERE user_username='$username' AND user_pass='$password' AND user_encrypt='$hashpass' AND user_status='1';";
...
{
    $this->db->reconnect();
    $qry="SELECT * FROM gimp_user WHERE user_id='$UserId'";
    $res = $this->db->query($qry);
